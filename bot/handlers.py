...
async def difficulty_to_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not topic or not difficulty_from:
        await callback.answer(
            "Пожалуйста, сначала выбери тему и минимальную сложность.", show_alert=True
        )
        return
    ...
    async with AsyncSessionLocal() as session:
        ...
        problems = await crud.get_random_by_tag_and_points_range(
            topic, difficulty_from, difficulty_to, limit=10
        )
    ...
    if not problems:
        ...
        text = "Вот задачи для тебя:\n\n" + "\n\n".join(
            f"{i + 1}. 🔗 [Задача {p.contest_id}{p.index}]"
            f"(https://codeforces.com/problemset/problem/{p.contest_id}/{p.index}) — *{p.name}*"
